...
export const requireRank = (
	user: IUser,
	role: Role,
	dept?: LeadershipDepartment | LeadershipDepartment[]
): boolean => {
	...
	if (!user.dept)
		throw error(
			403,
			'You do not have a leadership department assigned. Please contact your superior.'
		);
	if (typeof dept === 'string') {
		...
		throw error(
			...
			'This page can only be accessed by ' +
				(typeof dept === 'string' ? dept : dept.join(', ')) +
				' ' +
				role +
				's and above.'
		);
...
export const requireManageAttendance = (user: IUser) =>
	requireRank(user, Role.CORPORAL, [
		LeadershipDepartment.ADMINISTRATION,
		LeadershipDepartment.TRAINING
	]);
