    ...
    <Modal bind:open={showUserEditModal} title="Edit User" size="lg">
        <form method="POST" action="?/editUser">
            ...
            <p>
                ...
                Ensure that fields are consistent with the spreadsheets;
                otherwise, this may cause major errors with the system.
            </p>
            <div class="grid gap-6 md:grid-cols-2">
                <div>
                    <Label for="firstName">First Name</Label>
                    <Input type="text" id="firstName" name="firstName" bind:value={userEditModalSelection.firstName} required />
                </div>
                <div>
                    <Label for="lastName">Last Name</Label>
                    <Input type="text" id="lastName" name="lastName" bind:value={userEditModalSelection.lastName} required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <Select id="role" name="role" items={rankOptions} bind:value={userEditModalSelection.role} required />
                </div>
                <div>
                    <Label for="department">Leadership Department</Label>
                    <Select id="department" name="dept" items={deptOptions} bind:value={userEditModalSelection.dept} required />
                </div>
            </div>
