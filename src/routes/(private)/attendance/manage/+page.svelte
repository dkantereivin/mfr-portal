<script lang="ts">
	...
	const membersForRoles = (roles: Role[]) =>
		_.sortBy(
							...
							<div class="flex flex-col justify-center align-middle text-center">
								...
								{#if isFinalized}
									...
									<Tooltip placement="bottom">
										This training date has been finalized. Any further changes should be entered
										manually into the volunteer hours sheet.
									</Tooltip>
								{:else}
									...
									<Popover title="Date Currently Editable" placement="bottom">
										...
										<form method="POST" action="?/export">
											<input
												type="hidden"
												name="date"
												value={dayjs(date).tz().format('YYYY-MM-DD')}
											/>
											<div class="flex flex-row justify-center items-center space-x-4 mt-2">
												...
												<input
													type="number"
													name="hours"
													class="w-16 border border-gray-300 rounded-md px-2 py-2"
													placeholder="Hours"
													value={2}
												/>
												<button class="px-4 py-3 bg-red-500 text-white rounded-md hover:bg-red-600"
													>Export and Finalize</button
												>
											</div>
										</form>
									</Popover>
									...
									>
										<div class="flex flex-row justify-evenly">
											<form method="POST" class="contents">
												...
												<button
													formaction="?/create"
													class="px-2 py-1 text-green-500 rounded-md hover:bg-green-500 hover:text-white"
													>✅</button
												>
												<button
													formaction="?/delete"
													class="px-2 py-1 text-red-500 rounded-md hover:bg-red-500 hover:text-white"
													>❌</button
												>
											</form>
											<a
												href="/attendance/manage/{date}"
												class="px-2 py-1 text-blue-500 rounded-md hover:bg-blue-500 hover:text-white text-xl font-bold"
												>ⓘ</a
											>
										</div>
									</Popover>
