...
export const load = (async ({ url, locals }) => {
	...
	users.forEach((user) => {
		const attendanceByDate = _.groupBy(user.attendance, (attendance) =>
			parseUtc(attendance.time).tz().format('YYYY-MM-DD')
		);
	...
	const finalizedVector = await Promise.all(
		periodicTrainingDates.map(
			async (date) =>
				0 <
